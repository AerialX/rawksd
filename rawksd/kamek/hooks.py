...
class BranchInsnHook(Hook):
	...
	required_data = ['branch_type', 'target_func', 'src_addr_%CONFIG%']
	
	def __init__(self, builder, module, data):
		Hook.__init__(self, builder, module, data)
	
	def create_patches(self):
		target_func = self.builder._find_func_by_symbol(self.data['target_func'])
...
class AddFunctionPointerHook(Hook):
	...
	required_data = ['target_func', 'src_addr_%CONFIG%']
