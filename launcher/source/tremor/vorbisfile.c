...
extern long f_seek(void *f,int offset, int mode);
extern int f_close(void *f);
extern long  f_tell(void * f);
...
static void _seek_helper(OggVorbis_File *vf,ogg_int64_t offset){
  if(vf->datasource){ 
    (vf->callbacks.seek_func)(vf->datasource, (long) offset, SEEK_SET);
    vf->offset=offset;
...
static int _open_seekable2(OggVorbis_File *vf){
  ...
  vf->offset=vf->end=(ogg_int64_t )(vf->callbacks.tell_func)(vf->datasource);
...
static int _fseek64_wrap(FILE *f,ogg_int64_t off,int whence){
  if(f==NULL)return(-1);
  return f_seek(f,(long) off,whence);
}
...
ogg_int64_t ov_time_tell(OggVorbis_File *vf){
  ...
  return(time_total+(((ogg_int64_t)1000)*((ogg_int64_t)(vf->pcm_offset-pcm_total)))/ ((ogg_int64_t) vf->vi[link].rate));
}
