...
static const u8 InCo[4]={0xB,0xD,0x9,0xE};  /* Inverse Coefficients */
...
static u32 pack(const u8 *b)
{ /* pack bytes into a 32-bit Word */
    return ((u32)b[3]<<24)|((u32)b[2]<<16)|((u32)b[1]<<8)|(u32)b[0];
}


static void unpack(const u32 a,u8 *b)
...
static u8 bmul(const u8 x,const u8 y)
...
static u32 SubByte(const u32 a)
{
    u8 b[4];
    unpack(a,b);
    b[0]=fbsub[b[0]];
...
static u8 product(const u32 x,const u32 y)
...
void aes_decrypt2(u8 *iv, u8 *inbuf, u8 *outbuf, u64 len) {
  ...
  for (blockno = 0; blockno <= (len / sizeof(block)); blockno++) {
	...
	{
		block[i] ^= iv[i];
		iv[i] = inbuf[blockno * sizeof(block) + i];
		outbuf[blockno * sizeof(block) + i] = block[i];
	}
    //    debug_printf("Block %d output: ", blockno);
    //    hexdump(outbuf + blockno*sizeof(block), 16);
  }
